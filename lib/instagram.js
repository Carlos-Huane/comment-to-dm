...
/**
 * Lista los posts recientes de la cuenta con sus media_id.
 * Útil para saber qué ID agregar a content/post-links.json cuando publicás nuevo post.
 *
 * @param {number} limit — cuántos posts traer (max 25 recomendado)
 */
export async function getRecentMedia(limit = 25) {
  const result = await graphRequest("/me/media", {
    params: {
